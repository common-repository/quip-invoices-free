    ...
    <?php if ($type == 'invoice'): ?>
        ...
        <table class="form-table">
            ...
            <?php if ($invoice->recurring): ?>
                <tr valign="top">
                    <th scope="row">
                        <label><?php _e('Repeating Invoice', 'quip-invoices'); ?>:</label>
                    </th>
                    <td>
                        <p>
                        <?php if ($invoice->invoiceID == $invoice->baseInvoiceID):?>
                            <a href="<?php echo admin_url('admin.php?page=quip-invoices-recurring&type=invoice&id=' . $invoice->invoiceID); ?>" class="button">View Invoices</a>
                        <?php else: ?>
                            <a href="<?php echo admin_url('admin.php?page=quip-invoices-edit&type=invoice&id=' . $invoice->baseInvoiceID); ?>" class="button button-primary">Edit Parent</a>
                        <?php endif; ?>
                        </p>
                    </td>
                </tr>
        ...
        <?php else: ?>
            <table class="widefat">
                ...
                <tr>
                    ...
                    <th><?php _e('Payment ID', 'quip-invoices'); ?>*</th>
                </tr>
                </thead>
                <tbody>
                ...
                <?php foreach ($payments as $p): ?>
                    <tr>
                        ...
                        <td>
                            ...
                            else if ($p->paymentType == 5)
                            {
                                if ($p->paypalTransactionID) $type = "Paypal";
                                else $type = "Paypal" . "<div><small>(" . __('Manual', 'quip-invoices') . ")</small></div>";
                            }
                            echo $type;
                            ?>
                        </td>
                        <td>
                            ...
                            {
                                $paypalLink = "<a href='https://www.paypal.com'>" . $p->paypalTransactionID . "</a>";
                                echo $paypalLink;
                            }
                            else
                            {
                                _e('Payment ID for online payments only.', 'quip_invoices');
                            }
                            ?>
                        </td>
                    </tr>
    ...
    <?php elseif ($type == 'client'): ?>
        ...
        <table class="form-table">
            <tr valign="top">
                ...
                <td>
                    <span><strong><?php echo stripslashes($client->clientName); ?></strong></span><br/>
                    <?php $wpUser = get_user_by('id', $client->wpUserID); ?>
                    <?php if ($wpUser): ?>
                        <span>WordPress User: <a href="<?php echo admin_url('user-edit.php?user_id=' . $wpUser->ID) ?>"><?php echo $wpUser->display_name; ?></a></span>
                    <?php endif; ?>
                </td>
            </tr>
            <tr valign="top">
