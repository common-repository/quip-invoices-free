...
$subject = $options['emailDefaultQuoteSubject'];
$message = stripslashes(base64_decode($options['emailDefaultQuoteMessage']));
...
<div class="wrap">
    ...
    <?php if ($invoice->active == 0): ?>
        <div class="error"><p><?php  _e('DELETED QUOTE.  Data shown here for record keeping purposes.','quip-invoices'); ?></p></div>
    ...
    <?php endif; ?>
    <p><?php _e('Use this form to send the quote directly to your customer via email. The customer will receive an email containing a link allowing them to view the quote online.', 'quip-invoices'); ?>
        <br/>
    ...
    <?php if ($invoice->sent): ?>
        <div class="updated"><p><?php echo __('You last sent this quote on: ' . date('d F Y \a\t g:ia', strtotime($invoice->sent)) ,'quip-invoices'); ?></p></div>
    <?php endif; ?>
    <form action="" method="post" id="quip-invoices-send-invoice-form">
        <input type="hidden" name="action" value="quip_invoices_send_quote"/>
        <input type="hidden" name="invoiceID" value="<?php echo $invoice->invoiceID; ?>"/>
        <table class="form-table">
            <tr valign="top">
                <th scope="row">
                    <label><?php _e('Quote Details', 'quip-invoices'); ?>:</label>
                </th>
                <td>
                    <p>
                        <strong><?php _e('Quote Number', 'quip-invoices'); ?>: </strong>
                        <span><?php echo $invoice->invoiceNumber; ?></span><br/>
                        <strong><?php _e('Quote Date', 'quip-invoices'); ?>: </strong>
                        <span><?php echo date('l, d F, Y', strtotime($invoice->invoiceDate)); ?></span><br/>
                        <?php if ($invoice->dueDate): ?>
                            <strong><?php _e('Expiry Date', 'quip-invoices'); ?>: </strong>
                            <span><?php echo date('l, d F, Y', strtotime($invoice->dueDate)); ?></span><br/>
                        <?php endif; ?>
                        <strong><?php _e('Grand Total', 'quip-invoices'); ?>: </strong>
            ...
            <tr valign="top">
                ...
                <td>
                    ...
                    <p class="description"><?php _e('The email message body', 'quip-invoices'); ?>. <?php _e('Defaults to your', 'quip-invoices'); ?>
                        ...
                        <code>%%INVOICE_AMOUNT%%</code> - <?php _e('The quote grand total amount', 'quip-invoices'); ?><br/>
                        <code>%%INVOICE_LINK%%</code> - <?php _e('A link to view the quote online', 'quip-invoices'); ?><br/>
                        <code>%%COMPANY_DETAILS%%</code> - <?php _e('Your company details for this quote (Name, Address, Phone, Email)', 'quip-invoices'); ?><br/>
                    </p>
                </td>
            </tr>
            <tr valign="top">
            ...
            <tr valign="top">
                <th scope="row">
                    <label for="sendAdminCopy"><?php _e('Attach Quote PDF', 'quip-invoices'); ?>:</label>
                </th>
                <td>
                    <label class="radio">
                    ...
                    <p class="description"><?php _e('Attach a copy of the quote as a PDF to the email.', 'quip-invoices'); ?></p>
        ...
        <p class="submit">
            <button type="submit" class="button button-primary" <?php echo $demo; ?> ><?php _e('Send Quote', 'quip-invoices'); ?></button>
            <a href="<?php echo admin_url('admin.php?page=quip-invoices-quotes'); ?>" class="button"><?php _e('Cancel', 'quip-invoices'); ?></a>
