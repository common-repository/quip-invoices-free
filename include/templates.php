...
if (!function_exists('qu_in_invoice_js_vars'))
{
    ...
    function qu_in_invoice_js_vars($invoice)
    {
        ...
        return apply_filters('quip_invoices_invoice_js_vars', array(
            'ajaxurl' => admin_url('admin-ajax.php'),
            'pageurl' => site_url() . '?qinvoice=' . $invoice->hash,
            'symbol' => $localeStrings['symbol'],
...
if (!function_exists('qu_in_invoice_header'))
{
    ...
    function qu_in_invoice_header($invoice = false)
    {
        ...
        ?>
        <script type="text/javascript" src="<?php echo QUIP_INVOICES_JS_DIR ?>invoice.js?<?php echo QuipInvoices::$VERSION; ?>"></script>
        <script type="text/javascript">
            /* <![CDATA[ */
            var quip_invoices = <?php echo json_encode( (($invoice) ? qu_in_invoice_js_vars($invoice) : qu_in_standard_js_vars()) ); ?>;
            /* ]]> */
        </script>
    <?php
