...
class QuipInvoicesTableQuotes extends WP_List_Table
{
    function __construct()
    {
        parent::__construct(array(
            'singular' => __('Quote', 'quip-invoices'), //Singular label
            'plural' => __('Quotes', 'quip-invoices'), //plural label, also this well be one of the table css class
...
<p>
Upgrade to our Pro version to gain access to PDF versions of invoices & quotes and <i>much, much more...</i>
</p>
<BR>
<a href="https://bit.ly/3fCpzU4" target="_blank">Click here for more info...</a>
    ...
    function get_columns()
    {
        return $columns = array(
            ...
            'invoiceNumber' => __('Quote Number', 'quip-invoices'),
            'invoiceDate' => __('Date', 'quip-invoices'),
            'dueDate' => __('Expiry Date', 'quip-invoices'),
    ...
    function prepare_items()
    {
        ...
        $query = "SELECT * FROM " . $wpdb->prefix . 'qi_invoices' . " WHERE active=1 and type='quote' ";
                ...
                foreach ($columns as $column_name => $column_display_name)
                {
                    ...
                    switch ($column_name)
                    {
                        case "cb":
                            ...
                            break;
                        case "invoiceNumber":
                            $row = '<td ' . $attributes . '><strong><a href="' . admin_url('admin.php?page=quip-invoices-edit&type=quote&id=' . $rec->invoiceID) . '" >' . $rec->invoiceNumber . '</a></strong>';
                            ...
                            $row .= '<span><a href="' . admin_url('admin.php?page=quip-invoices-send&type=quote&id=' . $rec->invoiceID) . '" class="send-invoice">' . __('Send', 'quip-invoices') . '</a> | </span>';
                            $row .= '<span><a href="convert" data-id="' . $rec->invoiceID . '" class="convert-invoice">' . __('Convert to Invoice', 'quip-invoices') . '</a> | </span>';
                        ...
                        case "invoiceDate":
                            ...
                            break;
                        case "dueDate":
                            if (!$rec->dueDate) $dueDate = 'None';
                            else $dueDate = date('F jS Y', strtotime($rec->dueDate));
                            echo '<td ' . $attributes . '>' . $dueDate . '</td>';
                            break;
                        case "status":
                            $status = QuipInvoices::getInstance()->invoice->calculate_invoice_status($rec->invoiceID);
                            $col = '<td ' . $attributes . '>';
                            if ($status->viewed)
                                $col .= '<strong>Viewed</strong><div><small>' . __('on', 'quip-invoices') . ' ' . date('F jS \a\t g:ia', strtotime($rec->viewed)) . '</small></div>';
                            else if ($status->sent)
                                $col .= '<strong>Sent</strong><div><small>' . __('on', 'quip-invoices') . ' ' . date('F jS \a\t g:ia', strtotime($rec->sent)) . '</small></div>';
                            else
                                $col .= '<strong>New</strong>';
                            $col .= '</td>';
                            echo $col;
                            break;
                    }
