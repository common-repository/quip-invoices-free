...
class QuipInvoicesTableMyInvoices extends WP_List_Table
{
    ...
    function prepare_items()
    {
        ...
        $query = "SELECT * FROM {$wpdb->prefix}qi_invoices i JOIN {$wpdb->prefix}qi_clients c ON c.id = i.clientID WHERE i.active=1 AND i.type='invoice' AND c.wpUserID=$wpUserID";
        ...
        if (!empty($records))
        {
            ...
            foreach ($records as $rec)
            {
                ...
                foreach ($columns as $column_name => $column_display_name)
                {
                    ...
                    {
                        case "invoiceNumber":
                            $row = '<td ' . $attributes . '><strong><a href="' . site_url('?qinvoice=' . $rec->hash) . '" >' . $rec->invoiceNumber . '</a></strong>';
                            $row .= '<div class="row-actions visible">';
                            $row .= '<span><a href="' . site_url('?qinvoice=' . $rec->hash) . '" class="view-invoice">' . __('View', 'quip-invoices') . '</a> | </span>';
                            $row .= '</div>';
                            $row .= '</td>';
                            echo $row;
                            break;
