...
class QuipInvoicesTablePayments extends WP_List_Table
{
    ...
    function get_columns()
    {
        return $columns = array(
            ...
            'liveMode' => __('API Mode', 'quip-invoices'),
        );
    }
    ...
    function prepare_items()
    {
        ...
        if (!empty($_GET['s']))
        {
            ...
            $query .= " WHERE (amount LIKE '%$search%' OR stripePaymentID LIKE '%$search%' OR paypalTransactionID LIKE '%$search%')";
        }
        ...
        if (!empty($records))
        {
            ...
            foreach ($records as $rec)
            {
                ...
                foreach ($columns as $column_name => $column_display_name)
                {
                    ...
                    {
                        case "cb":
                            $cb = "<input type='checkbox' name='checked[]' value='" . $rec->id . "' id='checkbox_" . $rec->id . "' />";
                        ...
                        case "invoiceID":
                            ...
                            {
                                echo '<td ' . $attributes . '><strong>' . __('Deleted Invoice', 'quip-invoices') . '</strong></td>';
                            }
                            break;
                        case "amount":
                        ...
                        case "paymentType":
                            ...
                            if ($rec->paymentType == 1)
                            {
                                ...
                                $stripeLink .= "payments/" . $rec->stripePaymentID . "'>$rec->stripePaymentID</a>";
                            ...
                            else if ($rec->paymentType == 5)
                            {
                                $type = __('Paypal', 'quip-invoices');
                                $paypalLink = "<a href='https://www.paypal.com'>" . $rec->paypalTransactionID . "</a>";
                                $type .= '<div><small>' . $paypalLink . '</small></div>';
                            }
                        ...
                        case "paymentDate":
                            ...
                            break;
                        case "liveMode":
                            echo '<td ' . $attributes . '>' . ($rec->livemode == 1 ? 'Live' : 'Test')  . '</td>';
                            break;
                    }
                }
