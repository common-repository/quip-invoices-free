...
class QuipInvoicesAdminMenu
{
    ...
    public function admin_menu()
    {
        ...
        $submenu_page_title = __('Reports', 'quip-invoices');
        $submenu_title = __('Reports', 'quip-invoices');
        $submenu_slug = 'quip-invoices-reports';
        $submenu_function = array($this, 'display_reports');
        $menu_hook = add_submenu_page($menu_slug, $submenu_page_title, $submenu_title, $capability, $submenu_slug, $submenu_function);
        add_action('admin_print_scripts-' . $menu_hook, array($this, 'admin_scripts'));
        ...
        /**
         * Page just for clients to view their own invoices.  This will show up for subscriber role WP users
         * and only show invoices that are assigned to them.
         */
        $submenu_page_title = __('My Invoices', 'quip-invoices');
        $submenu_title = __('My Invoices', 'quip-invoices');
        $submenu_slug = 'quip-invoices-my-invoices';
        $submenu_function = array($this, 'display_my_invoices');
        $menu_hook = add_submenu_page($menu_slug, $submenu_page_title, $submenu_title, 'read', $submenu_slug, $submenu_function);
        add_action('admin_print_scripts-' . $menu_hook, array($this, 'admin_scripts'));
        ...
        $submenu_page_title = 'Repeating Invoices';
        $submenu_title = 'Repeating';
        $submenu_slug = 'quip-invoices-recurring';
        $submenu_function = array($this, 'display_recurring');
        $menu_hook = add_submenu_page(null, $submenu_page_title, $submenu_title, $capability, $submenu_slug, $submenu_function);
        add_action('admin_print_scripts-' . $menu_hook, array($this, 'admin_scripts'));
    ...
    public function admin_scripts()
    {
        ...
        wp_enqueue_script('quip-invoices-adminutils-js', plugins_url('/js/adminutils.js', dirname(__FILE__)), array('jquery'), QuipInvoices::$VERSION);
    ...
    public function display_settings()
    {
        ...
        $this->admin_enqueue_and_localize('settings', [], ['wp-color-picker']);
    ...
    public function display_invoices()
    {
        ...
        //load the table as required
        if (isset($_GET['tab']) && $_GET['tab'] == 'templates')
        {
            if (!class_exists('QuipInvoicesTableTemplates'))
            {
                require_once(QUIP_INVOICES_DIR . '/include/tables/quip-invoices-table-templates.php');
            }

            $templatesTable = new QuipInvoicesTableTemplates();
        ...
        // check if we're creating an invoice from a template
        if (isset($_GET['tab']) && $_GET['tab'] == 'create' && isset($_GET['template']))
        {
            $t = QuipInvoices::getInstance()->db->get_template($_GET['template']);
        }
    ...
    public function display_quotes()
    {
        ...
        if (!class_exists('QuipInvoicesTableInvoices'))
        {
            require_once(QUIP_INVOICES_DIR . '/include/tables/quip-invoices-table-quotes.php');
        }

        $table = new QuipInvoicesTableQuotes();
        $table->prepare_items();
    ...
    /**
     * Display my invoices page
     */
    public function display_my_invoices()
    {
        $this->admin_enqueue_and_localize('my_invoices');
        ...
        if (!class_exists('QuipInvoicesTableMyInvoices'))
        {
            require_once(QUIP_INVOICES_DIR . '/include/tables/quip-invoices-table-my-invoices.php');
        }

        $table = new QuipInvoicesTableMyInvoices();
        $table->prepare_items();

        include QUIP_INVOICES_DIR . '/view/my_invoices_page.php';
    }
    ...
    public function display_edit()
    {
        if ($this->validate_id_and_type())
        {
            ...
            {
                $template = QuipInvoices::getInstance()->db->get_template($itemID);
                if ($template)
                {
                    $this->admin_enqueue_and_localize('templates', array('edit' => true));
                    include QUIP_INVOICES_DIR . '/view/edit_template_page.php';
                }
                else
                    include QUIP_INVOICES_DIR . '/view/error_page.php';
            }
    ...
    /**
     * Display recurring page
     */
    public function display_recurring()
    {
        $this->admin_enqueue_and_localize('invoices');
        ...
        if (!class_exists('QuipInvoicesTableRecurring'))
        {
            require_once(QUIP_INVOICES_DIR . '/include/tables/quip-invoices-table-recurring.php');
        }

        $table = new QuipInvoicesTableRecurring();
        $table->prepare_items();

        include QUIP_INVOICES_DIR . '/view/recurring_page.php';
    }
    ...
    public function display_reports()
    {
        ...
        if (isset($_GET['year']) && filter_var($_GET['year'], FILTER_VALIDATE_INT))
            $year = filter_var(trim($_GET['year']), FILTER_SANITIZE_NUMBER_INT);

        $this->admin_enqueue_and_localize('reports', ['reportYear' => $year]);

        include QUIP_INVOICES_DIR . '/view/reports_page.php';
    }
     ...
     * @param array $extraScripts Any extra scripts to enqueue
     */
    private function admin_enqueue_and_localize($page, $extraData = array(), $extraScripts = array())
    {
        ...
        wp_enqueue_script("quip-invoices-admin-{$page}-js",
            ...
            array_merge(array('quip-invoices-adminutils-js'), $extraScripts),
            QuipInvoices::$VERSION);
    ...
    private function get_strings_for_page($page)
    {
        ...
        $strings['cancel'] = __('Cancel', 'quip-invoices');
        $strings['done'] = __('Done', 'quip-invoices');
